...
client = anthropic.Anthropic() #The call to Anthropic() calls for the env vairable ANTHROPIC-API-KEY
...
def write_cover_letter(
    ...
    # the input parameters are name,email company name and jd and 2 things gotten from the 2 agents: resume summary from agent1 and company_research from agent 2

    prompt = f"""Write a professional cover letter for {user_name} applying to 
                {company_name} for the role of {jd_summary.get('role_title', 'this position')} with the resume highlights of the candidate as {resume_highlights}.
